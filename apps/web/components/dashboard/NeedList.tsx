...
export default function NeedList({ needs, onNeedClick }: { needs: NeedNode[]; onNeedClick: (need: NeedNode) => void }) {
  ...
  return (
    <div className="flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm mt-2 flex-1 min-h-[200px]">
      <div className="px-4 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/60">
        ...
        <select
          className="bg-white text-xs px-2 py-1 rounded-lg text-gray-600 border border-gray-200 outline-none focus:border-[#115E54]/40"
          value={filter}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((need) => (
            <div
              ...
              className="group cursor-pointer p-3 mb-1.5 rounded-lg hover:bg-[#115E54]/5 transition-all border border-transparent hover:border-[#115E54]/15 hover:shadow-sm"
            >
              <div className="flex justify-between items-start mb-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-800 text-xs capitalize">{need.type}</span>
                </div>
